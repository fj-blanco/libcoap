...
/* Zephyr timing context - minimal replacement for mbedtls_timing_delay_context */
typedef struct {
    uint32_t start_time;
    uint32_t int_time;
    uint32_t fin_time;
...
static int zephyr_timing_get_delay(void *data)
{
    ...
    if (now >= ctx->fin_time) {
        return 2;   /* Final delay reached */
    }
    
    if (now >= ctx->int_time) {
        return 1;   /* Intermediate delay reached */
    }
    
    return 0;       /* No delay reached yet */
}
